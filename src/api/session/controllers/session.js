...
module.exports = {
  ...
  async update(ctx) {
    try {
      ...
      const updated = await updateSession(session.id, ctx.request.body);
  ...
  async delete(ctx) {
    try {
      ...
      await strapi.entityService.delete(SESSION_UID, session.id);
  ...
  async getWishlist(ctx) {
    try {
      ...
      return ctx.send({
        ...
        length: session?.wishlist.length,
      });
    } catch (err) {
      return ctx.badRequest(err.message);
    }
  ...
  async removeWishlist(ctx) {
    try {
      ...
      const existingWishlist = session.wishlist || [];
  ...
  async getViewed(ctx) {
    try {
      ...
      return ctx.send({
        success: true,
        viewed: session?.lastViewed.slice(0, 5) || [],
      });
    } catch (err) {
      return ctx.badRequest(err.message);
    }
  ...
  async addSearch(ctx) {
    try {
      ...
      searched = searched.filter((item) => item.searchQuery !== searchQuery);
